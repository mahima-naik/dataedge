...
# Step 3: Update VAD start sensitivity - fix: change from HIGH to BALANCED
sed -i 's/GEMINI_LIVE_VAD_START_SENSITIVITY=START_SENSITIVITY_HIGH/GEMINI_LIVE_VAD_START_SENSITIVITY=START_SENSITIVITY_BALANCED/g' /root/app/.env
sed -i 's/GEMINI_LIVE_VAD_START_SENSITIVITY=START_SENSITIVITY_LOW/GEMINI_LIVE_VAD_START_SENSITIVITY=START_SENSITIVITY_BALANCED/g' /root/app/.env

# Step 4: Update VAD end sensitivity - fix: change from HIGH to BALANCED
sed -i 's/GEMINI_LIVE_VAD_END_SENSITIVITY=END_SENSITIVITY_HIGH/GEMINI_LIVE_VAD_END_SENSITIVITY=END_SENSITIVITY_BALANCED/g' /root/app/.env
sed -i 's/GEMINI_LIVE_VAD_END_SENSITIVITY=END_SENSITIVITY_LOW/GEMINI_LIVE_VAD_END_SENSITIVITY=END_SENSITIVITY_BALANCED/g' /root/app/.env

echo "  ✓ VAD sensitivity updated to BALANCED"
