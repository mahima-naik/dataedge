...
def _verify_password_sync(email: str, password: str) -> Optional[str]:
    """Verify user credentials and return role, or None if invalid (sync)."""
...
async def verify_password(email: str, password: str) -> Optional[str]:
    """Verify user credentials — runs bcrypt in a thread to avoid blocking the event loop."""
    return await asyncio.to_thread(_verify_password_sync, email, password)
