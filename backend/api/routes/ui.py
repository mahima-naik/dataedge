...
async def serve_demo_explicit():
    return await html_whitelisted_file("demo.html")


@router.get("/factory")
async def serve_factory_alias():
    """Matches post-login redirect in ``login.html`` (factory role → ``/factory``)."""
    return await html_whitelisted_file("factory.html")


@router.get("/factory.html", include_in_schema=False)
async def serve_factory_explicit():
    return await html_whitelisted_file("factory.html")
