...
async def whatsapp_proxy_status():
    ...
    if not session_id or not api_key:
        return {"enabled": True, "error": "OPENWA_SESSION_ID or OPENWA_API_KEY not configured"}
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            resp = await client.get(
                f"{_openwa_base()}/api/sessions/{session_id}",
...
async def whatsapp_proxy_qr():
    ...
    session_id = settings.openwa_session_id.strip()
    if not session_id or not api_key:
        raise HTTPException(status_code=503, detail="OPENWA_SESSION_ID or OPENWA_API_KEY not configured")
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.get(
                f"{_openwa_base()}/api/sessions/{session_id}/qr",
                headers={"X-API-Key": api_key},
            )
...
@router.get("/api/whatsapp/proxy/logs")
async def whatsapp_proxy_logs(
    role: str = "data_edge",
    status: str | None = None,
    limit: int = 50,
):
    """View WhatsApp message log entries."""
    from core.storage import get_whatsapp_logs_by_role
    logs = await get_whatsapp_logs_by_role(role=role, status=status, limit=limit)
