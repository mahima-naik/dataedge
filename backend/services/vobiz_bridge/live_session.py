...
async def handle_vobiz_ws_live(
    ...
    async def _vobiz_ws_ping_task() -> None:
        try:
            while True:
    ...
    _ping_task = asyncio.create_task(_vobiz_ws_ping_task())
    _background_tasks.add(_ping_task)
    _ping_task.add_done_callback(_background_tasks.discard)
    ...
    _gemini_ws_connect_task: Optional[asyncio.Task] = asyncio.create_task(
        ws_client.connect(
            ...
            ping_interval=10,
            close_timeout=2,
            additional_headers=_gemini_extra_headers,
        ).__aenter__()
    )
    ...
    defer_gemini_until_scripted = False  # Connect Gemini Live in parallel with greeting
        ...
        async with _ReuseGeminiWS() as gem:
            ...
            async def pump_vobiz_to_gemini() -> None:
                ...
                while True:
                    ...
                    if raw is None:
                        _vobiz_ws_closed = True
                        if not gemini_setup_complete.is_set():
                            logger.info("Vobiz inbound queue closed BEFORE Gemini setupComplete — waiting up to 15s for Gemini to connect")
                            _wait_t0 = time.perf_counter()
                            while not gemini_setup_complete.is_set() and (time.perf_counter() - _wait_t0) < 15.0:
                                await asyncio.sleep(0.1)
                            if gemini_setup_complete.is_set():
                                logger.info("Vobiz WS closed but Gemini setupComplete received — continuing session")
                            else:
                                logger.info("Vobiz WS closed and Gemini setupComplete still not received after 15s — ending session")
                        else:
                            logger.info("Vobiz inbound queue closed (Gemini already set up)")
                        return
                    try:
                        msg = json.loads(raw)
                    except Exception:
                    ...
                    if len(_outbuf) >= _BATCH_BYTES or (now >= _next_flush and _outbuf):
                        try:
                            ...
                            if hasattr(ws, 'client_state') and ws.client_state in _closed_states:
                                if not gemini_setup_complete.is_set():
                                    logger.info("Vobiz WS closed during playout BEFORE Gemini setupComplete — parking mixer until setup or 15s timeout")
                                    _park_t0 = time.perf_counter()
                                    while not gemini_setup_complete.is_set() and (time.perf_counter() - _park_t0) < 15.0:
                                        await asyncio.sleep(0.1)
                                    if gemini_setup_complete.is_set():
                                        logger.info("Mixer: Gemini setupComplete received after Vobiz disconnect — exiting mixer (no Vobiz to send to)")
                                    else:
                                        logger.info("Mixer: Gemini setupComplete still not received after 15s — exiting mixer")
                                else:
            ...
            # Always prompt Gemini to speak when there is an opening line (even with scripted PCM).
            needs_live_opening_nudge = bool((opening_line or "").strip())
            # When scripted PCM greeting was already played (prior_16k_queue > 0 at connect),
            # skip the "Speak your opening line" nudge — it contradicts the system prompt
            # which says "DO NOT REPEAT greeting". Only the continuation nudge should fire.
            needs_live_opening_nudge = needs_live_opening_nudge and _prior_opening_bytes_at_connect == 0
            logger.info(
